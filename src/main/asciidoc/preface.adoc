...
The Spring Data Neo4j project applies core Spring concepts to the development of solutions using the Neo4j graph data store.
We provide _"repositories"_ as a high-level abstraction for storing and querying documents.
You will notice similarities to the JPA/Hibernate support in the Spring Framework.
...
If you plan on altering the version of the Neo4j-OGM make sure it is only in the 3.0.0+ release family.
