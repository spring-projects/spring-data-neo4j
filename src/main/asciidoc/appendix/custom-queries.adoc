...
In real-life this is sometimes not enough and there are nodes deeper in the graph that needs to get added to the resulting instance.
...
Because there are multiple paths that lead from 'The Matrix' to another movie, the result will be still not a single record.
...
It looks a little complicated at first sight, but the `reduce` function allows us to flatten the nodes and relationships from various paths.
