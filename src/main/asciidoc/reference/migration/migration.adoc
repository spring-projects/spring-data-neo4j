= Migration Guide from SDN3 to SDN4
:numbered:
== Package changes
...
The `@Query` annotation is only supported in Spring Data Neo4j.
...
|`query`
|`query`
|Return type changed from `Result` to be `Iterable` // todo consolidate to List
