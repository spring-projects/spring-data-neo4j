...
This can be accomplished by supplying the username and password when constructing a `RemoteServer`,
or by embedded them into the URL such as `http://username:password@localhost:7474`.

.Passing connection credentials when constructing a RemoteServer
[source,java]
----
@Bean
public Neo4jServer neo4jServer() {
...
@Bean
public Neo4jServer neo4jServer() {
    return new RemoteServer("http://username:password@localhost:7474",username,password);
}
----

Otherwise, Spring Data Neo4j 4 reads System properties `username` and `password` and supplies them with each request to the Neo4j database.
