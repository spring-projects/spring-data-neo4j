...
[Note]
`Neo4jTemplate`'s functionality can all be found on the OGM `Session` object. Users of SDN 4.2 are recommended to Autowire `Session`
instead of `Neo4jTemplate` as the integration into SDN is better. `Neo4jTemplate` has been left in for those users who are on SDN 4.1.x and below.
