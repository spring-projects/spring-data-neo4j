...
[NOTE]
A custom cypher provided by using the `@Query` annotation will automatically force a read/write transaction.
This is a known limitation because Neo4j-OGM ignores the `readOnly` flag if custom cypher is provided.
