...
Those POJOs are easier to handle and might be use as Data Transfer Objects (DTOs) as they are not ttached to a `Session` and don't participate in any lifecycle.
...
[CAUTION]
Note that if you open a read only transaction from a service method and then call a mutating method that is marked as read/write your transaction semantics will always be defined by the outermost transaction. 
...
In this mode checks are taken whether outer and inner transactions can work together or not.
