...
Spring Data Neo4j (version 4) does not currently provide facilities for handling that setup out of the box.


As this is a seeding, migration or maintenance effort, it should be handled as part of managing your Neo4j environment or
by the group responsible for the database maintenance.

To find out more about how to set up and maintain Indexes and Constrains in Neo4j see the http://neo4j.com/docs/developer-manual/current/#cypherdoc-labels-constraints-and-indexes[Neo4j Reference Manual].
