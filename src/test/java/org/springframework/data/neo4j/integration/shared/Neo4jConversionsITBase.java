...
import org.springframework.data.neo4j.integration.shared.ThingWithAllAdditionalTypes.SomeEnum;
...
public abstract class Neo4jConversionsITBase {
	...
	static {
		...
		try {
			hlp.put("aURL", new URL("http://www.fake.com"));
		...
		try (Session session = neo4jConnectionSupport.getDriver().session()) {
			session.writeTransaction(w -> {
				...
				ID_OF_SPATIAL_TYPES_NODE = w.run("CREATE (n:SpatialTypes) SET "
						...
						+ " n.car2d = point({x: 10, y: 20})," + " n.car3d = point({x: 30, y: 40, z: 50})" + " RETURN id(n) AS id",
