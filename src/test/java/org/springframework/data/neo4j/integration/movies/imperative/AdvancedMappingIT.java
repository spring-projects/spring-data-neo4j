...
class AdvancedMappingIT {
	...
	void nestedSelfRelationshipsFromCustomQueryShouldWork(@Autowired Neo4jTemplate template) {
		...
		assertThat(optionalPartner).hasValueSatisfying(p -> {
			...
			Map<String, Organisation> org1Childs = org1.getOrganisations()
				.stream()
			...
			Map<String, Organisation> org3Childs = org3.getOrganisations()
				.stream()
				.collect(Collectors.toMap(Organisation::getCode, Function.identity()));
	...
	void directionAndTypeLessPathMappingShouldWork(@Autowired Neo4jTemplate template) {
		...
		List<Person> people = template.findAll("MATCH p=(:Person)-[]-(:Person) RETURN p", Collections.emptyMap(),
				Person.class);
	...
	void mappingOfAPathWithOddNumberOfElementsShouldWorkFromStartToEnd(@Autowired Neo4jTemplate template) {
		...
		Map<String, Movie> movies = template.findAll("""
		...
		// This is the actual test for the original issue… When the end node of a segment
		// is not taken into account, Emil is not an actor
	...
	void pathMappingWithoutAdditionalInformationShouldWork(@Autowired Neo4jTemplate template) {
		...
		assertThat(people).hasSize(4)
			...
			.contains("Kevin Bacon", "Jessica Thompson", "Angela Scope"); // Two paths
																			// lead there,
																			// one with
																			// Ron Howard,
																			// one with
		...
		assertThat(people).element(2)
			...
			.satisfies(movies -> assertThat(movies).extracting(Movie::getTitle).containsExactly("The Da Vinci Code"));
	}
	...
	void findPreservesOrderFromResultAscInRelationshipList(@Autowired MovieRepository repository) {
		assertThat(repository.findMatrixWithSortedAscActors().getActors()).extracting("person")
			.extracting("name")
	...
	void findPreservesOrderFromResultDescInRelationshipList(@Autowired MovieRepository repository) {
		assertThat(repository.findMatrixWithSortedDescActors().getActors()).extracting("person")
			.extracting("name")
	...
	void createPropertyFilterPathCorrectly(@Autowired Neo4jTemplate neo4jTemplate) {
		...
		Movie foundMovie = neo4jTemplate
			.findOne("MATCH (m:Movie{title:'Test'}) return m", Collections.emptyMap(), Movie.class)
			.get();
