...
class ReactiveIssuesIT extends TestBase {
	...
	void testNewRelation(@Autowired ReactiveSkuRepository skuRepo) {
		...
		skuRepo.save(new Sku(0L, "A"))
			...
			.zipWith(skuRepo.save(new Sku(3L, "D")))
			.flatMap(t -> {
		...
		skuRepo.findById(bId.get()).doOnNext(b -> assertThat(b.getRangeRelationsIn()).hasSize(1)).flatMap(b -> {
			b.rangeRelationTo(cRef.get(), 1, 1, RelationType.MULTIPLICATIVE);
			return skuRepo.save(b);
		}).as(StepVerifier::create).assertNext(b -> {
			assertThat(b.getRangeRelationsIn()).hasSize(1);
			assertThat(b.getRangeRelationsOut()).hasSize(1);
		}).verifyComplete();

		skuRepo.findById(aId.get()).as(StepVerifier::create).assertNext(a -> {
	...
	void testNewRelationRo(@Autowired ReactiveSkuRORepository skuRepo) {
		...
		skuRepo.findOneByName("a new name").as(StepVerifier::create).verifyComplete();

		skuRepo.findOneByName("B").doOnNext(b -> {
			assertThat(b.getRangeRelationsIn()).hasSize(1);
	...
	void entitiesWithAssignedIdsSavedInBatchMustBeIdentifiableWithTheirInternalIds(
			@Autowired ReactiveApplicationRepository applicationRepository, @Autowired Driver driver,
			@Autowired BookmarkCapture bookmarkCapture) {
		applicationRepository.saveAll(Collections.singletonList(createData()))
			.as(StepVerifier::create)
			.expectNextCount(1L)
			.verifyComplete();
	...
	void entitiesWithAssignedIdsMustBeIdentifiableWithTheirInternalIds(
			@Autowired ReactiveApplicationRepository applicationRepository, @Autowired Driver driver,
			@Autowired BookmarkCapture bookmarkCapture) {
		applicationRepository.save(createData()).as(StepVerifier::create).expectNextCount(1L).verifyComplete();
		assertSingleApplicationNodeWithMultipleWorkflows(driver, bookmarkCapture);
	}
	...
	void relationshipsStartingAtEntitiesWithAssignedIdsShouldBeCreated(
		...
		createData((applications, workflows) -> {
			applicationRepository.saveAll(applications).as(StepVerifier::create).expectNextCount(2L).verifyComplete();
	...
	void relationshipsStartingAtEntitiesWithAssignedIdsShouldBeCreatedOtherDirection(
			@Autowired ReactiveWorkflowRepository workflowRepository, @Autowired Driver driver,
			@Autowired BookmarkCapture bookmarkCapture) {
		createData((applications, workflows) -> {
			workflowRepository.saveAll(workflows).as(StepVerifier::create).expectNextCount(2L).verifyComplete();
	...
	@Test
	@Tag("GH-2498")
	void shouldNotDeleteFreshlyCreatedRelationships(@Autowired Driver driver,
			@Autowired ReactiveNeo4jTemplate template) {

		Group group = new Group();
		group.setName("test");
		template.findById(1L, Device.class).flatMap(d -> {
	...
	@Test
	@Tag("GH-2533")
	void projectionWorksForDynamicRelationshipsOnSave(@Autowired ReactiveGH2533Repository repository,
			@Autowired ReactiveNeo4jTemplate neo4jTemplate) {
		createData(repository).flatMap(rootEntity -> repository.findByIdWithLevelOneLinks(rootEntity.id))
			.flatMap(rootEntity -> neo4jTemplate.saveAs(rootEntity,
					EntitiesAndProjections.GH2533EntityNodeWithOneLevelLinks.class))
			.flatMap(
					rootEntity -> neo4jTemplate.findById(rootEntity.getId(), EntitiesAndProjections.GH2533Entity.class))
			.as(StepVerifier::create)
			.assertNext(entity -> {
	...
	@Test
	@Tag("GH-2533")
	void saveRelatedEntityWithRelationships(@Autowired ReactiveGH2533Repository repository,
			@Autowired ReactiveNeo4jTemplate neo4jTemplate) {
		createData(repository).flatMap(rootEntity -> repository.findByIdWithLevelOneLinks(rootEntity.id))
			.flatMap(rootEntity -> neo4jTemplate.saveAs(rootEntity,
					EntitiesAndProjections.GH2533EntityNodeWithOneLevelLinks.class))
	...
	@Test
	@Tag("GH-2905")
	void saveSingleEntities(@Autowired ReactiveFromRepositoryV1 fromRepositoryV1,
			@Autowired ReactiveToRepositoryV1 toRepositoryV1, @Autowired Driver driver) {
		...
		toRepositoryV1.save(bugTargetV1).flatMapMany(to1 -> {
			...
			var from1 = BugFromV1.builder()
				...
				.reli(BugRelationshipV1.builder().target(to1).comment("F1<-T1").build())
				.build();
			// This is the key to solve 2905 when you had the annotation previously, you
			// must maintain both ends of the bidirectional relationship.
	...
	@Test
	@Tag("GH-2906")
	void saveSingleEntitiesToLeaf(@Autowired ReactiveFromRepository fromRepository,
			@Autowired ReactiveToRepository toRepository, @Autowired Driver driver) {
	...
	@Test
	@Tag("GH-2906")
	void saveSingleEntitiesToContainer(@Autowired ReactiveFromRepository fromRepository,
			@Autowired ReactiveToRepository toRepository, @Autowired Driver driver) {
	...
	@Test
	@Tag("GH-2906")
	void saveSingleEntitiesViaServiceToContainer(@Autowired ReactiveFromRepository fromRepository,
			@Autowired ReactiveToRepository toRepository, @Autowired Driver driver) {
		...
		toRepository.save(to1).flatMapMany(x -> {
			...
			return Flux.concat(saveGH2906Entity(from1, uuid, fromRepository, toRepository),
	...
	@Test
	@Tag("GH-2906")
	void saveTwoSingleEntitiesViaServiceToContainer(@Autowired ReactiveFromRepository fromRepository,
			@Autowired ReactiveToRepository toRepository, @Autowired Driver driver) {
	...
	@Test
	@Tag("GH-2906")
	void saveSingleEntitiesViaServiceToLeaf(@Autowired ReactiveFromRepository fromRepository,
			@Autowired ReactiveToRepository toRepository, @Autowired Driver driver) {
	...
	@Test
	@Tag("GH-2906")
	void saveTwoSingleEntitiesViaServiceToLeaf(@Autowired ReactiveFromRepository fromRepository,
			@Autowired ReactiveToRepository toRepository, @Autowired Driver driver) {
	...
	private Mono<BugFrom> saveGH2906Entity(BugFrom from, String uuid, ReactiveFromRepository fromRepository,
			ReactiveToRepository toRepository) {
	...
	void shouldSupportGeoResult(@Autowired ReactiveLocatedNodeRepository repository) {
		...
		List<GeoResult<LocatedNode>> nodes = repository.findAllAsGeoResultsByPlaceNear(Place.SFO.getValue())
			.collectList()
			.block();
		...
		nodes = repository
			.findAllByPlaceNear(Place.MINC.getValue(),
