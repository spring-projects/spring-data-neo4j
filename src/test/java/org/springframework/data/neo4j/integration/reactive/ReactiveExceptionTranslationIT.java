...
/**
 * In case you are wondering about the class naming: It is indeed an integration test. But
 * if we name it `IT`, it will be run the SDN Jar and only that, our migration scripts
 * won't be included in that Jar, and migrations can't be found. This is likely due to the
 * parent build, and it's not worth investigating much here unless we decide to have more
 * tests based on migrations.
 *
...
class ReactiveExceptionTranslationIT {
	...
	void exceptionsFromClientShouldBeTranslated(@Autowired ReactiveNeo4jClient neo4jClient) {
		neo4jClient.query("CREATE (:SimplePerson {name: 'Tom'})")
			.run()
			.then()
			.as(StepVerifier::create)
			.verifyComplete();
		...
		neo4jClient.query("CREATE (:SimplePerson {name: 'Tom'})")
			.run()
			.as(StepVerifier::create)
			.verifyErrorMatches(this.aTranslatedException);
	}
	...
	void exceptionsFromRepositoriesShouldBeTranslated(@Autowired SimplePersonRepository repository) {
		...
		repository.save(new SimplePerson("Tom")).as(StepVerifier::create).verifyErrorMatches(this.aTranslatedException);
	}
	...
	void exceptionsOnRepositoryBeansShouldBeTranslated(@Autowired CustomDAO customDAO) {
		...
		customDAO.createPerson().as(StepVerifier::create).verifyErrorMatches(this.aTranslatedException);
	}
	...
	static class Config extends Neo4jReactiveTestConfiguration {
		...
		@Bean
		CustomDAO customDAO(ReactiveNeo4jClient neo4jClient) {
		...
		@Bean
		ReactivePersistenceExceptionTranslationPostProcessor persistenceExceptionTranslationPostProcessor() {
			return new ReactivePersistenceExceptionTranslationPostProcessor();
		}

		@Bean
		Migrations migrations(@Autowired Driver driver) {
			return new Migrations(MigrationsConfig.builder().withLocationsToScan("classpath:/data/migrations").build(),
					driver);
		}
	...
	static class CustomDAO {
		...
		Mono<ResultSummary> createPerson() {
			return this.neo4jClient
				.delegateTo(rxQueryRunner -> Flux.from(rxQueryRunner.run("CREATE (:SimplePerson {name: 'Tom'})"))
