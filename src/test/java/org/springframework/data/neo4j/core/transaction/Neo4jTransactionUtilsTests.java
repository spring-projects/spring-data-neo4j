 ...
 */
class Neo4jTransactionUtilsTests {

	@CsvSource(nullValues = "n/a", delimiter = '|', value = {
	...
	@ParameterizedTest
	void formatOngoingTxInAnotherDbErrorMessageShouldWork(String cdb, String rdb, String cu, String ru,
			String expected) {

		DatabaseSelection currentDatabaseSelection = StringUtils.hasText(cdb) ? DatabaseSelection.byName(cdb)
				: DatabaseSelection.undecided();
		DatabaseSelection requestedDatabaseSelection = StringUtils.hasText(rdb) ? DatabaseSelection.byName(rdb)
				: DatabaseSelection.undecided();
		UserSelection currentUserSelection = StringUtils.hasText(cu) ? UserSelection.impersonate(cu)
				: UserSelection.connectedUser();
		UserSelection requestedUserSelection = StringUtils.hasText(ru) ? UserSelection.impersonate(ru)
				: UserSelection.connectedUser();

		String result = Neo4jTransactionUtils.formatOngoingTxInAnotherDbErrorMessage(currentDatabaseSelection,
				requestedDatabaseSelection, currentUserSelection, requestedUserSelection);
