 ...
 */
class PropertyTraverserTests {

	private final Neo4jMappingContext ctx;

	PropertyTraverserTests() {
	...
	void shouldTraverseAll() {
		...
		traverser.traverse(Movie.class, (path, property) -> includedProperties.put(path.toString(),
				property.isAssociation() && !property.isAnnotationPresent(TargetNode.class)));
	...
	void onlyMovieDirectFields() {
		...
		PropertyTraverser traverser = new PropertyTraverser(this.ctx);
		Map<String, Boolean> includedProperties = new TreeMap<>();
		traverser.traverse(Movie.class, (path, property) -> !property.isAssociation(),
	...
	void onlyDirectors() {
		...
		traverser.traverse(Movie.class,
				(path, property) -> property.getName().equals("directors")
						|| (path.toDotPath().startsWith("directors.") && property.getName().equals("name")),
				(path, property) -> includedProperties.put(path.toString(), property.isAssociation()));
