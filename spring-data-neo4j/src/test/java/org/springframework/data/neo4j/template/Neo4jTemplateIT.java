...
public class Neo4jTemplateIT extends MultiDriverTestClass {
	...
	/**
	 * While this may seem trivial, some of these tests actually used to fail when run against a database containing
	 * unrelated data.
	 */
	...
	public void shouldExecuteArbitraryReadQuery() {
		...
		Result queryResults = this.template.query("MATCH (u:User)-[r]->(m:Movie) RETURN AVG(r.stars) AS avg",
	...
	public void shouldThrowExceptionForExecuteQueryThatReturnsResults() {
		this.template.query("CREATE (g1:Genre {name:'Comedy'}), (g2:Genre {name:'Action'}) return g1",
				Collections.EMPTY_MAP);
	}
	...
	public void shouldDeleteExistingEntitiesByGraphId() {
		Long genreId = graphDatabaseService
				.execute("CREATE (t:Genre {name:'Thriller'}), (r:Genre {name:'RomCom'}) RETURN id(r) AS gid")
	...
	public void shouldReturnQueryStats() {
		QueryStatistics stats = this.template
				.query("CREATE (a:Actor {name:'Keanu Reeves'}) CREATE (m:Movie {title:'The Matrix'}) "
	...
	public void shouldReturnSchemaQueryStats() {
		...
		stats = this.template
				.query("CREATE CONSTRAINT ON (movie:Movie) ASSERT movie.title IS UNIQUE", Collections.EMPTY_MAP)
				.queryStatistics();
		assertEquals(1, stats.getConstraintsAdded());

		stats = this.template.query("DROP CONSTRAINT ON (movie:Movie) ASSERT movie.title is UNIQUE", Collections.EMPTY_MAP)
				.queryStatistics();
	...
	public void shouldReturnQueryStatsForQueryWithParams() {
		QueryStatistics stats = this.template
				.query(
						...
						map("actorName", "Keanu Reeves", "movieTitle", "THe Matrix"))
				.queryStatistics();
		assertTrue(stats.containsUpdates());
		assertEquals(2, stats.getNodesCreated());
		assertEquals(3, stats.getPropertiesSet());
	...
	public void shouldAllowResultsToBeReturnedFromModifyingQueries() {
		Result results = this.template.query(
				"CREATE (a:Actor {name:{actorName}}) CREATE (m:Movie {title:{movieTitle}}) "
						+ "CREATE (a)-[:ACTED_IN {role:'Neo'}]->(m) return a.name as actorName, m.title as movieName",
				map("actorName", "Keanu Reeves", "movieTitle", "The Matrix"));
