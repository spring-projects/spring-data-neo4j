...
class ContainsComparisonBuilder extends FilterBuilder {
	...
	public List<Filter> build(Stack<Object> params) {
		...
		if (Collection.class.isAssignableFrom(part.getProperty().getTypeInformation().getType())) {
			...
			FilterFunction<Object> comparison = new ContainsAnyComparison(valueToBeContained);
			containingFilter = new Filter(nestedAttributes.isEmpty() ? propertyName() : nestedAttributes.getLeafPropertySegment(), comparison);
		} else {
			containingFilter = new Filter(nestedAttributes.isEmpty() ? propertyName() : nestedAttributes.getLeafPropertySegment(), ComparisonOperator.CONTAINING, valueToBeContained);
		}
